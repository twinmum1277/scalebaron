...
class CompositeApp:
    ...
    def setup_widgets(self):
        ...
        # --- Top Button ---
        ttk.Button(control_frame, text="Select Input Folder", command=self.select_input_folder).pack(pady=(5, 15))
        ...
        color_options = sorted(m for m in plt.colormaps() if not m.endswith('_r'))
        ttk.Label(grid_frame, text="Color Scheme:").grid(row=4, column=0, sticky="e", padx=5, pady=2)
        self.color_dropdown = ttk.Combobox(grid_frame, textvariable=self.color_scheme, values=color_options, state='readonly')
        self.color_dropdown.grid(row=4, column=1, padx=5, pady=2)
        ...
        ttk.Button(button_frame, text="Preview", command=self.preview_composite).grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(button_frame, text="Save", command=self.save_composite).grid(row=0, column=1, padx=5, pady=5)
    ...
    def scan_input_folder(self):
        self.element_choices.clear()
        found_elements = set()
        found_samples = set()
        pattern = os.path.join(self.input_dir, "*matrix.xlsx")
        files = sorted(glob.glob(pattern))

        for f in files:
            match = re.match(r"(.+)[ _]([A-Za-z]{1,2}\d{2,3})_(ppm|CPS) matrix\.xlsx", os.path.basename(f))
            if match:
                sample, element_mass, _ = match.groups()
                found_elements.add(element_mass)
                found_samples.add(sample)

        if found_elements:
            sorted_elements = sorted(found_elements)
            self.element_choices.extend(sorted_elements)
            self.element_dropdown['values'] = sorted_elements
            if sorted_elements:
                self.element.set(sorted_elements[0])
            self.log_print("\n📦 Found Elements:")
        ...
        if found_samples:
            self.log_print("\n🔍 Found Samples:")
            for s in sorted(found_samples):
                self.log_print(f" - {s}")

        if not found_elements:
            self.log_print("⚠️ No valid matrix files found.")
