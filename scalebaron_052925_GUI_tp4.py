...
# 2025-05-28 constrained summary statistics.csv to 2 decimal places, added colorbar output at line 378.
# 2025-05-29 Fixed backrgound color to the zero value.
...
class CompositeApp:
    def __init__(self, master):
        ...
        self.unit_label = tk.StringVar(value="ppm") # Default to ppm, updated based on file type
    ...
    def setup_widgets(self):
       ...
       # Select Input Folder at the top
        ...
        ttk.Entry(grid_frame, textvariable=self.pixel_size, width=10).grid(row=1, column=1, padx=5, pady=2)

        # Multiple sizes checkbox (renamed)
        ttk.Checkbutton(grid_frame, text="Multiple sizes?", variable=self.use_custom_pixel_sizes).grid(row=2, column=1, columnspan=1, sticky="w", pady=(0, 10))

        # Generate pixel size template button
        ttk.Button(grid_frame, text="Pixel Sizes", command=self.generate_pixel_size_template).grid(row=2, column=2, columnspan=2, padx=5, pady=2)
        ...
        # Step 1: Summarize
        ttk.Label(button_frame, text="Step 1:").grid(row=0, column=0, padx=5, pady=(5, 0), sticky="w")
        ttk.Button(button_frame, text="Summarize Data", command=self.load_data).grid(row=1, column=0, padx=5, pady=(0, 10), sticky="ew")

        # Step 2: Preview
        ttk.Label(button_frame, text="Step 2:").grid(row=2, column=0, padx=5, pady=(0, 0), sticky="w")
        ...
        ttk.Label(button_frame, text="Step 3:").grid(row=4, column=0, padx=5, pady=(0, 0), sticky="w")
        ttk.Button(button_frame, text="Select Output Folder", command=self.select_output_folder).grid(row=5, column=0, padx=5, pady=(0, 10), sticky="ew")

        # Step 4: Export
        ttk.Label(button_frame, text="Step 4:").grid(row=6, column=0, padx=5, pady=(0, 0), sticky="w")
        ttk.Button(button_frame, text="Generate Maps", command=self.save_composite).grid(row=7, column=0, padx=5, pady=(0, 10), sticky="ew")
    ...
    def generate_composite(self, preview=False):
        ...
        rgba = cmap(0)
        bg_color = (rgba[0], rgba[1], rgba[2], 1.0)
        ...
        for i, (matrix, label, (adjusted_height, adjusted_width)) in enumerate(zip(self.matrices, self.labels, self.adjusted_dimensions)):
            ...
            subplot_ax.axis('off')
            subplot_fig.savefig(subplot_path, dpi=300, bbox_inches='tight', facecolor=bg_color)
            plt.close(subplot_fig)

        # Create colorbar for subplots
        fig, ax = plt.subplots(figsize=(4, 0.4))
        fig.subplots_adjust(bottom=0.5)
        cmap = plt.get_cmap(self.color_scheme.get())
        rgba = cmap(0)
        bg_color = (rgba[0], rgba[1], rgba[2], 1.0) # Use colormap’s zero color as background
        norm = Normalize(vmin=0, vmax=self.scale_max.get())
        cb = cm.ScalarMappable(norm=norm, cmap=cmap)
        cbar = fig.colorbar(cb, cax=ax, orientation='horizontal')
        cbar.set_label(self.unit_label.get(), fontsize=10)
        cbar.ax.tick_params(labelsize=8)

        colorbar_path = os.path.join(self.output_dir, self.element.get(), 'subplots', 'colorbar.png')
        os.makedirs(os.path.dirname(colorbar_path), exist_ok=True)
        fig.savefig(colorbar_path, dpi=300, bbox_inches='tight', transparent=True)
        plt.close(fig)

        self.log_print(f"✅ Saved shared colorbar to {colorbar_path}")
